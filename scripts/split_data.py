...
def validate_split(
    train_df: pd.DataFrame, test_df: pd.DataFrame, target_column: str = "class"
) -> dict:
    """
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Output directory (default: same as input)"
    )
    ...
    if args.input_path:
        ...
        # Default: look for data relative to script location
    ...
    train_df, test_df = hash_split(
        df, test_size=args.test_size, hash_column=args.hash_column, target_column=args.target_column
    )
    ...
    # Also write a copy for the Streamlit app (used as sample data)
    script_dir = Path(__file__).resolve().parent
    project_root = script_dir.parent
    app_data_dir = project_root / "app" / "data"
    app_data_dir.mkdir(parents=True, exist_ok=True)
    sample_path = app_data_dir / "sample_data.csv"
    test_df.to_csv(sample_path, index=False)

    print("\nFiles saved:")
    print(f"   {train_path}")
    print(f"   {test_path}")
    print(f"   {sample_path}  (Streamlit app sample data)")
