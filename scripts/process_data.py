...
# german.data is not redistributed in this repository. It is fetched from UCI on
# first run. The checksum is pinned because split_data.py splits on a row hash,
# so a different source file would silently change every reported metric.
...
def ensure_raw_data(raw_path: Path) -> None:
    """Download german.data if it is not present, and verify its checksum."""
    if not raw_path.exists():
        print(f"Downloading german.data from {RAW_URL}")
        raw_path.parent.mkdir(parents=True, exist_ok=True)
        try:
            urllib.request.urlretrieve(RAW_URL, raw_path)  # noqa: S310
        except OSError as exc:
            raise RuntimeError(
                f"Could not download the dataset from {RAW_URL} ({exc}).\n"
                f"Download it manually and save it to {raw_path}."
            ) from exc
    ...
    if digest != RAW_SHA256:
        raise RuntimeError(
            ...
            "The published metrics assume the original UCI file. Delete the file to re-download."
        )
...
def create_german_credit_csv():
    ...
    if len(rows[0]) != len(col_names):
        raise ValueError(f"Expected {len(col_names)} columns, got {len(rows[0])}")
